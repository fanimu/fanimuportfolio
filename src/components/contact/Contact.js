    ...
    <section id="contact" className="w-full py-20 border-b-[1px] border-b-black">
      ...
      <div className="w-full">
        <div className="w-full h-auto flex flex-col lgl:flex-row justify-between">
          <ContactLeft />
          <div className="w-full lgl:w-[60%] h-full py-10 bg-gradient-to-r from-[#1e2024] to-[#23272b] p-4 lgl:p-8 flex flex-col gap-8 rounded-lg shadow-shadowOne">
            <form className="w-full flex flex-col gap-4 lgl:gap-6 py-2 lgl:py-5">
              ...
              <div className="flex flex-col gap-4">
                ...
                <input
                  ...
                  value={email}
                  className={`${errMsg === 'Please give your email!' && 'outline-designColor'} ${errMsg === 'Give a valid email!' && 'outline-designColor'} contactInput`}
                  type="email"
                />
              </div>
              <div className="flex flex-col gap-4">
                <p className="text-sm text-gray-400 uppercase tracking-wide">Subject</p>
                <input onChange={(e) => SetSubject(e.target.value)} value={subject} className={`${errMsg === 'Please give your subject!' && 'outline-designColor'} contactInput`} type="email" />
              </div>
              <div className="flex flex-col gap-4">
                <p className="text-sm text-gray-400 uppercase tracking-wide">Message</p>
                <textarea onChange={(e) => setMessage(e.target.value)} value={message} className={`${errMsg === 'Message is required!' && 'outline-designColor'} contactInput`} cols="30" rows="8"></textarea>
              </div>
              <div className="w-full">
